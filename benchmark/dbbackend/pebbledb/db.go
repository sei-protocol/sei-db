...
func (b *Batch) set(tombstone uint64, key, value []byte) error {
	prefixedKey := MVCCEncode(key, b.version)
...
func (db *Database) Get(targetVersion uint64, key []byte) ([]byte, error) {
	prefixedVal, err := getMVCCSlice(db.storage, key, targetVersion)
	if err != nil {
...
func getMVCCSlice(db *pebble.DB, key []byte, version uint64) ([]byte, error) {
