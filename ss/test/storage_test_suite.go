...
func (s *StorageTestSuite) TestDatabaseBug() {
	...
	s.Require().NoError(DBApplyChangeset(db, 0, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val000")}))
	s.Require().NoError(DBApplyChangeset(db, 1, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val001")}))
	s.Require().NoError(DBApplyChangeset(db, 2, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val002")}))
	s.Require().NoError(DBApplyChangeset(db, 3, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val003")}))
	s.Require().NoError(DBApplyChangeset(db, 4, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val004")}))
	s.Require().NoError(DBApplyChangeset(db, 5, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val005")}))
	s.Require().NoError(DBApplyChangeset(db, 6, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val006")}))
	s.Require().NoError(DBApplyChangeset(db, 7, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val007")}))
	s.Require().NoError(DBApplyChangeset(db, 8, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val008")}))
	s.Require().NoError(DBApplyChangeset(db, 9, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val009")}))
	s.Require().NoError(DBApplyChangeset(db, 10, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val010")}))
	s.Require().NoError(DBApplyChangeset(db, 11, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val011")}))
	s.Require().NoError(DBApplyChangeset(db, 12, storeKey1, [][]byte{[]byte("key000")}, [][]byte{[]byte("val012")}))

	for i := int64(1); i <= 13; i++ {
		bz, err := db.Get(storeKey1, i, []byte("key000"))
		s.Require().NoError(err)
		s.Require().Equal(fmt.Sprintf("val%03d", i), string(bz))
	}
}
